...
static inline int scir(int t, const char *msg, int errval) {
	if (t == errval) {
		perror("error in system call");
		exit(1);
	}
...
#define SC(v,e) SCIE(v, __stringify(v),e);
#define sc(v,e) scie(v, __stringify(v),e);
#define sr(v,e) scir(v, __stringify(v),e);
...
// here come real calls
#define c_system(a) sc(system(a),-1)
#define c_sched_getparam(a,b) sc(sched_getparam(a,b),-1)
#define c_sched_getscheduler(a) sr(sched_getscheduler(a),-1)

// kernel log handling functions
static inline void klog_clear(void) {
	c_system("sudo dmesg -c > /dev/null");
}
...
static inline void print_scheduling_info() {
	...
	c_sched_getparam(0,&myparam);
	int scheduler;
	scheduler=c_sched_getscheduler(0);
	printf("==================================\n");
